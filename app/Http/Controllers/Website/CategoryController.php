...
class CategoryController extends Controller
{
    ...
    {
        $model = $this->category->where('slug', $slug)->where('status', 1)->first();
        if (!$model) {
            abort(404);
        }
        $query = $this->product->with('city')->where('product_category_id', $model->id)->where('status', 1)->where('is_verified', 1);
