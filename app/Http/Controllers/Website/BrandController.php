...
class BrandController extends Controller
{
    ...
    public function index(){
        $brands = $this->brand->where('status', 1)->get();
        return view('website.brand',compact('brands'));
    }
    public function show(Request $request, $slug)
    {
        // dd($request->all());
        $model = $this->brand->where('slug', $slug)->where('status', 1)->first();
        if (!$model) {
            abort(404);
        }
        $query = $this->product->with('city')->where('product_brand_id', $model->id)->where('status', 1)->where('is_verified', 1);
