...
@section('content')
    ...
    <form action="{{ route('admin.user-type.update', $model->id) }}" method="post" id="form"
                        ...
                        {{-- <div class="mb-1">
                            ...
                            @foreach ($modules as $item)
                                ...
                                <input type="checkbox" name="{{ $item->alias }}[]" class="form-check-input" value="add"
                                    id="{{ $item->alias }}-add" {{Helper::isUserTypeAccessed($model,$item->alias,'add') ? 'checked' : ''}}>
                                ...
                                <input type="checkbox" name="{{ $item->alias }}[]" class="form-check-input" value="edit"
                                    id="{{ $item->alias }}-edit" {{Helper::isUserTypeAccessed($model,$item->alias,'edit') ? 'checked' : ''}}>
                                ...
                                <input type="checkbox" name="{{ $item->alias }}[]" class="form-check-input" value="delete"
                                    id="{{ $item->alias }}-delete" {{Helper::isUserTypeAccessed($model,$item->alias,'delete') ? 'checked' : ''}}>
                                ...
                                <input type="checkbox" name="{{ $item->alias }}[]" class="form-check-input" value="view"
                                    id="{{ $item->alias }}-view" {{Helper::isUserTypeAccessed($model,$item->alias,'view') ? 'checked' : ''}}>
                                ...
                                <input type="checkbox" name="{{ $item->alias }}[]" class="form-check-input"
                                    value="changeStatus" id="{{ $item->alias }}-changeStatus" {{Helper::isUserTypeAccessed($model,$item->alias,'changeStatus') ? 'checked' : ''}}>
                                                ...
                                                <div class="d-flex">
                                                    ...
                                                    <div class="form-check me-2 me-lg-5">
                                                        <input class="form-check-input access" type="checkbox"
                                                            ...
                                                            id="{{ $item->alias }}-changeStatus" {{Helper::isUserTypeAccessed($model,$item->alias,'changeStatus') ? 'checked' : ''}}/>
                    ...
                    <div class="card-body">
                        <div class="form-check form-switch">
                            <input type="checkbox" name="status" class="form-check-input" id="status"
                                ...
                                {{ old('status', $model->is_active) == ConstantHelper::STATUS_ACTIVE ? 'checked' : '' }}>
