                ...
                if (models) {
                    models.forEach(function(item) {
                        $('#product_model_id').append($("<option/>", {
                            value: item.id,
                            text: item.title,
                            selected: "{{ old('product_model_id', $model->product_model_id) }}" ==
                                item.id ? true : false,
                        }));
                    });
                }
...
@section('content')
    ...
    <section class="horizontal-wizard">
        <div class="bs-stepper horizontal-wizard-example">
            ...
            <form action="{{ route('admin.product.update', $model->uuid) }}" method="post" id="form"
                                ...
                                <select name="city_id" id="city_id" class="form-select select2" required>
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ old('city_id', $model->city_id) == $city->id ? 'selected' : '' }}>
                                            {{ $city->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="product_category_id" id="product_category_id"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('product_category_id', $model->product_category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->title }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="product_brand_id" id="product_brand_id" class="form-select select2"
                                    ...
                                    @foreach ($brands as $brand)
                                        <option value="{{ $brand->id }}"
                                            {{ old('product_brand_id', $model->product_brand_id) == $brand->id ? 'selected' : '' }}>
                                            {{ $brand->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-1">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" name="discount_amount" id="discount_amount" class="form-control"
                                    value="{{ old('discount_amount', $model->discount_amount) }}" />
                            </div>
                        </div>
                        <div class="row mb-1">
                            <div class="col-md-4">
                                ...
                                <div class="form-check form-switch">
                                    <input type="checkbox" name="delivery" class="form-check-input" id="delivery"
                                        ...
                                        {{ old('delivery', $model->delivery) == ConstantHelper::STATUS_ACTIVE ? 'checked' : '' }}>
                            ...
                            <div class="col-md-4">
                                ...
                                <div class="form-check form-switch">
                                    <input type="checkbox" name="negotiable" class="form-check-input" id="negotiable"
                                        value="{{ ConstantHelper::STATUS_ACTIVE }}"
                                        {{ old('negotiable', $model->negotiable) == ConstantHelper::STATUS_ACTIVE ? 'checked' : '' }}>
                            ...
                            <div class="mb-1 row">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div id="main_image_preview">
                                        <div class="img"
                                            style="background-image: url({{ asset('/storage/' . $model->image) }}); background-size: cover ; background-position: center;">
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class=" mb-1 row">
                                ...
                                <div class="d-flex row">
                                    @foreach ($model->images as $image)
                                        <div style="position: relative; background-image: linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)),url({{ asset('/storage/thumbs/' . $image->file) }}); background-size:cover; height:100px; width:100px;"
                                            class="m-2 deleteimage col-3">
